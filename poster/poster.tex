...
\headerbox{Runtime}{name=BoxFive, column=1, span=2, below=BoxFour}{
  \center
  {\large Runntimes for solving the function $\displaystyle\prod_{i=1}^{n}{x_i\cdot sin(x_i)}$}\\
...
\headerbox{Function Complexity}{name=BoxSix, column=1, span=2, below=BoxFive, above=bottom}{
  \begin{tabular}{p{\graphWidth}p{.95\linewidth-2\graphWidth}p{\graphWidth}}
    ...
    \item Utilizing seperable partitions of the function can be usable
      ...
      \item This is only applicable if the function is seperable
      \end{enumerate}
    \end{enumerate}
...
\headerbox{Python Complications}{name=BoxSeven, column=3}{
  ...
  %% drawbacks.
  Python does not support cocurrent threads, only concurrent processes. To compound on this there
  are inconsistent costs for operations.
  \begin{itemize}\compresslist
  \item \emph{Ctype}, an unchangable list of build in types, varaibles are fast to share while
    most other types are orders of magnatude slower.
  \item Share namespace handling is via communication with a sentinal process.
